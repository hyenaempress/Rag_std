...
Django 기반의 Retrieval-Augmented Generation (RAG) 챗봇으로, 문서를 업로드하고 내용에 기반한 질문응답을 제공합니다.
 response_updateVer.01.02 버전 기반 빌드, 벡터 DB 제외 청킹, 가중치 세팅 상태에서 벡터 DB 설치 
...
### 2025-08-25 오후 업데이트 (v1.1)
- **ChromaDB 하이브리드 RAG 구축 완료** 🎉
 response_updateVer.01.02 베이스 업데이트
  1. **ChromaDB 설치 및 설정**
    - jhgan/ko-sroberta-multitask 한국어 임베딩 모델 사용
    - 영속적 로컬 저장소 (./chroma_db)
    - 179개 문서 벡터화 완료
  2. **하이브리드 RAG 엔진 구현**
    - 기존 키워드 검색 (TF-IDF 기반) 유지
    - 새로운 벡터 검색 (의미적 유사성) 추가
    - 스마트한 점수 결합 (벡터 0.6 + 키워드 0.4 가중치)
    - 중복 제거 및 신뢰도 보너스
    - ChromaDB → 메모리 자동 로드 기능
  3. **완전한 통합**
    - 기존 Django views.py 자동 연동
    - 새 문서 업로드 시 자동으로 양쪽 저장소에 저장
    - 마이그레이션 기능 완비
  4. **검증된 성능**
    - 의미적 유사성 검색 정상 작동 ✅
    - "파이썬 웹 개발" → Django 문서 찾기 성공 ✅
    - "벡터DB가 뭐야?" → 정확한 기술 설명 제공 ✅
    - 하이브리드 점수 계산 및 순위 정렬 완료 ✅

### 2025-08-25 저녁 업데이트 (v1.1.1) - 인코딩 안정성 강화
- **🔧 CP949 인코딩 문제 완전 해결**
  ```
  문제: 'cp949' codec can't encode character '\u25e6' in position 23
  해결: 5단계 안전 처리 시스템 구축
  ```
  1. **강화된 특수문자 처리 시스템**
    - 100+ 특수문자 → 안전한 ASCII 문자 자동 변환
    - 불릿 포인트: • ▪ ▫ ◦ → "- " 
    - 중요 표시: ★ ☆ → "중요: "
    - 수학 기호: × ÷ ≠ → "x" "/" "!="
    - 그리스 문자: α β γ → "alpha" "beta" "gamma"
  
  2. **_ensure_encoding_safety() 함수 추가**
    - CP949 인코딩 실시간 검증
    - 문자별 안전성 체크
    - 단계별 폴백 메커니즘
    
  3. **_ultra_safe_clean() 최종 방어선**
    - 문자별 CP949 호환성 테스트
    - 불가능한 문자 → 공백 대체
    - ASCII 폴백 보장
    
  4. **ChromaDB → 메모리 로드 시 전처리**
    ```python
    def _load_chroma_to_memory(self):
        # 179개 문서를 안전하게 메모리로 로드
        clean_text = self._remove_problematic_chars(doc_text)
        doc_obj = Document(page_content=clean_text, metadata=metadata)
    ```
    
  5. **검증 완료**
    -  벡터 검색 활성화: ChromaDB에 179개 문서 저장됨
    -  메모리 로드 완료: 하이브리드 검색을 위해 179개 문서 메모리 로드
    -  인코딩 문제 해결: CP949 호환 응답 생성 성공
    -  하이브리드 검색: 벡터 + 키워드 검색 결합 작동
    -  안전한 응답: 특수문자 제거 및 인코딩 안전성 보장
